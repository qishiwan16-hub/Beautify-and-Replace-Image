...
批量下载会先正常保存 ZIP；完成结果和直链显示在插件内部页面，不再使用受当前美化影响的外部弹窗。后端 v1.2.0 及以上可同时生成约 24 小时有效的下载直链，方便复制到其他浏览器。批量导入支持直接选择本插件下载的 ZIP 或普通 ZIP，并按压缩包内图片文件名匹配替换位置。下载、导入和链接替换会在实际处理完成后显示酒馆通知。关闭插件面板会取消仍在进行的后端或扩展更新检测，但不会中断已经开始的扩展更新。
