...
创作模式提供批量下载、批量导入本地图片和批量链接导入。批量链接按图片序号逐行映射；某一行填写“空”时会保留对应图片，不执行替换。链接替换使用插件内独立页面，避免当前美化干扰弹窗布局；替换只更新当前主题 CSS 编辑内容，仍需在酒馆主题界面手动保存。

批量下载会先正常保存 ZIP；后端 v1.2.0 及以上可同时生成约 24 小时有效的下载直链，方便复制到其他浏览器。批量导入支持直接选择本插件下载的 ZIP 或普通 ZIP，并按压缩包内图片文件名匹配替换位置。下载、导入和链接替换会在实际处理完成后显示酒馆通知。关闭插件面板会取消仍在进行的后端或扩展更新检测，但不会中断已经开始的扩展更新。
